 ...
 * 
 * UPDATE (_v2): Added functionality to work in case of non-multichannel images and fixed the bugs with the colours
...
print("Introduce the analysis settings and the image properties specific for your image, \n open the image first to obtain these parameters if you do not know");
  ...
  Dialog.addChoice("Is the image a multichannel data set? :", multi_Channel);
  ...
  //Get the variables from the dialog box to be used in the program (needs to be done in order)
   multi_Channel = Dialog.getChoice();
...
//Make sure the different channels are in different windows (only if the data is a multichannel image)
	if(multi_Channel == "YES"){
		run("Split Channels");
	
	}else if(multi_Channel == "NO"){
		print("No multichannel image, but program continues OK");
	}
...
//Fix the problem with the images names and rename them to continue
	if(BRIGHT == "YES"){
		if(multi_Channel == "YES"){
		print("Yes multichannel and it has brightfield");
			selectWindow("C"+channel_BF+"-"+fileNameExt);
		...
		//cell wall labelled cells and provide an accurate size profile (as opposed to the aberrations from brightfield images)
			selectWindow("C"+channel_F+"-"+fileNameExt);		
			rename("Red");
		}else if(multi_Channel == "NO"){
			print("No multichannel but only has brightfield");
				selectWindow(fileNameExt);
				rename("BF"); // renames the selected channel to BF
		// We wish to perform area calculations on the red channel cells, as they are the (NHS ester) 
		//cell wall labelled cells and provide an accurate size profile (as opposed to the aberrations from brightfield images)
...
save( dir1 + fileName+"_"+image_colour+"_Z_projection.tif"); 
...
//Select correct image based on image colour entered by the user then Change name of image to be able to save the correct file for ilastik
if (image_colour == "red"){
...
if (image_colour == "green"){
	
selectWindow("MAX_Red (green)");
name1 = fileName+"_ilastik";
rename(name1);
}

if(image_colour == "blue"){

selectWindow("MAX_Red (blue)");
name1 = fileName+"_ilastik";
rename(name1);
}
